...
export function ChatSystem({ user, accessToken, onNavigate }: ChatSystemProps) {
  ...
  const fetchGroups = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-98798d2b/chat/groups`,
        {
          headers: {
            'Authorization': `Bearer ${accessToken}`
          }
  ...
  const fetchMessages = async () => {
    ...
    try {
      let url = `https://${projectId}.supabase.co/functions/v1/make-server-98798d2b/chat/messages?`;
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-98798d2b/chat/messages`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
