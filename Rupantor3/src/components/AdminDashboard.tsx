...
export function AdminDashboard({ user, accessToken, onNavigate }: AdminDashboardProps) {
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingEvent 
        ? `https://${projectId}.supabase.co/functions/v1/make-server-98798d2b/events/${editingEvent.id}`
        : `https://${projectId}.supabase.co/functions/v1/make-server-98798d2b/events`;
  ...
  const handleCreateInstruction = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingInstruction
        ? `https://${projectId}.supabase.co/functions/v1/make-server-98798d2b/instructions/${editingInstruction.id}`
        : `https://${projectId}.supabase.co/functions/v1/make-server-98798d2b/instructions`;
  ...
  const toggleInstructionLock = async (instructionId: string) => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-98798d2b/instructions/${instructionId}/toggle-lock`,
        {
          method: 'PATCH',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
