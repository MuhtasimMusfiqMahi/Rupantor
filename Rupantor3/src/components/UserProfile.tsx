...
export function UserProfile({ email, currentUser, accessToken, onNavigate }: UserProfileProps) {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      console.log('API URL:', `https://${projectId}.supabase.co/functions/v1/server/users/by-email/${encodeURIComponent(email)}`);
      
      const userResponse = await fetch(
        `https://${projectId}.supabase.co/functions/v1/server/users/by-email/${encodeURIComponent(email)}`,
        {
          headers: accessToken ? {
