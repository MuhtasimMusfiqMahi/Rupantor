...
export function VolunteerDashboard({ user, accessToken, onNavigate }: VolunteerDashboardProps) {
  ...
  const updateStatus = async (instructionId: string, status: string) => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-98798d2b/instructions/${instructionId}`,
        {
          method: 'PATCH',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
  ...
  const addUpdate = async (instructionId: string) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-98798d2b/instructions/${instructionId}/update`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
