...
app.get('/', (c) => {
  return c.json({ 
    ...
    message: 'Use /make-server-98798d2b/* routes'
  });
});

// Health check endpoint
app.get('/make-server-98798d2b/health', (c) => {
...
// Signup
app.post('/make-server-98798d2b/signup', async (c) => {
  try {
...
// Get user by email
app.get('/make-server-98798d2b/users/by-email/:email', async (c) => {
  try {
...
// Get all events
app.get('/make-server-98798d2b/events', async (c) => {
  try {
...
app.post('/make-server-98798d2b/events', async (c) => {
  try {
...
app.patch('/make-server-98798d2b/events/:id', async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
...
// Like event
app.post('/make-server-98798d2b/events/:id/like', async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
...
// Comment on event
app.post('/make-server-98798d2b/events/:id/comment', async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
...
app.post('/make-server-98798d2b/events/:id/register', async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
...
app.post('/make-server-98798d2b/events/:id/unregister', async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
...
app.post('/make-server-98798d2b/events/:id/register-guest', async (c) => {
  try {
    const eventId = c.req.param('id');
...
app.post('/make-server-98798d2b/events/:id/unregister-guest', async (c) => {
...
// Get instructions
app.get('/make-server-98798d2b/instructions', async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
...
app.post('/make-server-98798d2b/instructions', async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
...
app.patch('/make-server-98798d2b/instructions/:id', async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
...
app.post('/make-server-98798d2b/instructions/:id/update', async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
...
app.patch('/make-server-98798d2b/instructions/:id/toggle-lock', async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
...
app.get('/make-server-98798d2b/volunteers', async (c) => {
  try {
    const user = await verifyAuth(c.req.header('Authorization'));
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
